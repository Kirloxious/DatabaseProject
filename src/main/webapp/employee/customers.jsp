...
<%
	for (Customer c : new MySQLConnection().getAllCustomers()) {
		...
		<label>Registered Date: <input type="date" name="date" value="<%out.print(date);%>" form="cust<%out.print(c.getSsn());%>"></label><br>
		<%
		int i = 0;
		...
		<input type="submit" name="add_address" value="Add" form="add_address<%out.print(c.getSsn());%>">
		<hr>
		<%
	}
%>
